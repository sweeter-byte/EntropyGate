'''
CHAIR evaluator — inlined from the project-root ``benchmark/evaluators/chair_evaluator.py``
so that the ``edge/`` package is self-contained.
...
class CHAIR(object):
    ...
    def compute_chair(self, cap_file, image_id_key, caption_key):
        ...
        avg_len = (0.01*len_caps/num_caps) if num_caps > 0 else 0.
