...
# =============================================================================
# Step 2: fp16 (no quantization) + 500 samples
# Purpose: Fair comparison with CRoPS paper results (fp16), isolate 4bit effect
# =============================================================================
...
MASTER_LOG="${LOG_DIR}/chair_fp16_${TIMESTAMP}.log"

echo "============================================================" | tee -a "${MASTER_LOG}"
echo "CHAIR (fp16, 500 samples) — $(date)" | tee -a "${MASTER_LOG}"
...
for METHOD in "${METHODS[@]}"; do
    EXPERIMENT_NAME="${METHOD}_chair_fp16_${TIMESTAMP}"

    echo "" | tee -a "${MASTER_LOG}"
    echo "------------------------------------------------------------" | tee -a "${MASTER_LOG}"
    echo "Method: ${METHOD} | CHAIR | fp16" | tee -a "${MASTER_LOG}"
    echo "------------------------------------------------------------" | tee -a "${MASTER_LOG}"
    ...
    echo "${METHOD}/chair_fp16 done." | tee -a "${MASTER_LOG}"
done
