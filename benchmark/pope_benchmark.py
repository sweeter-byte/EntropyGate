...
Expected POPE data format (JSONL):
    {"question_id": 1, "image": "COCO_val2014_000000xxx.jpg", "text": "Is there a dog in the image?", "label": "yes"}

See: https://github.com/AoiDragon/POPE
"""
...
class PopeBenchmarkDataset:
    def __init__(
        ...
        pope_path: str,
        coco_image_dir: str,
        pope_splits: list[str] | None = None,
    ):
        """
        Args:
            pope_path: Directory containing POPE JSONL files
                       (e.g. coco_pope_random.json, coco_pope_popular.json, coco_pope_adversarial.json)
            coco_image_dir: Directory containing COCO val2014 images
            pope_splits: List of splits to evaluate. Defaults to all three.
        """
    ...
    def _load_split(self, split: str) -> list[dict]:
        """Load a single POPE split from JSONL file."""
    ...
    def get_test_dataset(self) -> list[dict]:
        """Load all configured splits and return a combined sample list."""
        all_samples = []
    ...
    def evaluate(self, results_path: str, dump_results: bool = True) -> dict:
        ...
        # TP/FP/FN for precision/recall (treating "yes" as positive)
