...
class AmberBenchmarkDataset:
    ...
    def _load_query_data(self) -> list[dict]:
        ...
        for raw_sample in data:
            ...
            if sample_id is None:
                raise KeyError(
                    f"Could not find a sample identifier in AMBER record keys: {list(raw_sample.keys())}"
                )

            prompt = self._get_first(
                raw_sample,
                ["query", "prompt", "question", "instruction"],
                self.default_prompt,
            )
    ...
    def _resolve_image_path(self, raw_sample: dict, sample_id: int) -> str:
        image_value = self._get_first(
            raw_sample,
            ["image", "image_path", "img", "filename", "file_name"],
        )
        ...
        candidate_names.extend(
            [
                f"AMBER_{sample_id}.jpg",
                f"AMBER_{sample_id}.jpeg",
                f"AMBER_{sample_id}.png",
            ]
        )
        ...
        raise FileNotFoundError(
            f"Could not resolve image for AMBER sample {sample_id}. "
            f"Tried candidates: {candidate_names} under {self.image_dir}"
        )
    ...
    def evaluate(
        ...
        if not inference_script.exists():
            raise FileNotFoundError(
                f"Could not find AMBER evaluator script at {inference_script}"
            )

        # Convert to absolute path so it works regardless of subprocess cwd
        ...
        if proc.returncode != 0:
            raise RuntimeError(
                f"AMBER evaluation failed with exit code {proc.returncode}.\n{output}"
            )
