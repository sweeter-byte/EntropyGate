...
def _compat_causal_mask(model, attention_mask, inputs_embeds, cache_position,
    ...
    """Build causal mask across transformers versions. Called ONCE before the loop."""
...
def _apply_2d_mask_to_4d(causal_mask_4d, augmented_mask_2d, dtype):
    """Apply a 2D augmented attention mask onto an existing 4D causal mask.

    causal_mask_4d: (batch, 1, q_len, kv_len) or None
    augmented_mask_2d: (batch, kv_len) with 1=keep, 0=mask

    Returns a new 4D mask with masked positions set to a large negative value.
    """
    if causal_mask_4d is None:
        # No causal mask (e.g. full attention) — build one from the 2D mask
        batch, kv_len = augmented_mask_2d.shape
        # Expand 2D -> 4D: (batch, 1, 1, kv_len)
        mask_4d = augmented_mask_2d[:, None, None, :].to(dtype=dtype)
        # Convert 0/1 to 0/-inf
        mask_4d = (1.0 - mask_4d) * torch.finfo(dtype).min
        return mask_4d

    # causal_mask_4d exists: (batch, 1, q_len, kv_len) or (batch, heads, q_len, kv_len)
    batch, mask_len = augmented_mask_2d.shape
    kv_len = causal_mask_4d.shape[-1]

    # Pad augmented_mask_2d if the KV cache has grown beyond the original mask length.
    # New cache positions are padded with 1 (attend) so they are not masked out.
    if mask_len < kv_len:
...
def _is_v5_decoder_layer(decoder_layer):
    """Check if decoder layer uses v5.x style (returns tensor, not tuple)."""
    import inspect
...
def _run_decoder_layer_with_attn(decoder_layer, hidden_states, causal_mask,
                                 ...
                                 **extra_kwargs):
    """Run decoder layer and capture attention weights across versions.

    Returns (hidden_states, attn_weights_or_None).
    In v5.x, decoder_layer.forward no longer accepts output_attentions and
    returns a plain tensor.  We use a forward hook on self_attn to grab
    the attention weights that are otherwise discarded.
    """
    captured_attn = {}
    ...
    if _is_v5_decoder_layer(decoder_layer):
        # v5.x path: optionally register hook to capture attention weights
        hook = None
        ...
        # v5.x returns a plain tensor, not a tuple
        ...
        # Old transformers: returns tuple (hidden_states, attn_weights, ...)
...
def forward(
    ...
    **kwargs,
    ...
    # Store the base causal mask for reuse inside the loop
    ...
    # Determine the last layer index that needs attention weights for mask ops.
    # After this layer, we can skip attention weight computation entirely.
    max_attn_layer = 0
    ...
    for layer_idx, decoder_layer in enumerate(self.layers[: self.config.num_hidden_layers]):
        ...
        # Only compute attention weights for layers the mask operations need.
        # This saves ~1.4 GiB for a 32-layer model (30 layers × ~48 MiB each).
        ...
        if self.gradient_checkpointing and self.training:
            ...
            # Apply the 2D augmented mask on top of the base 4D causal mask
            if _HAS_CREATE_CAUSAL_MASK:
            ...
            # Free attention weights from memory once mask operations are done
